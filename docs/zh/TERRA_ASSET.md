...
CW20 或 Terra 原生代币可以中继到以太坊/BSC 网络。
...
- [Terra Asset Bridge](#terra-asset-bridge)
   ...
   - [BSC (Binance Smart Chain) 上的 BEP20 合约](#bep20-contracts-on-bsc-binance-smart-chain)
   - [HRC20 和谐合约](#hrc20-contracts-on-harmony)
   - [Terra Denoms and Contracts](#terra-denoms-and-contracts)
   - [使用说明](#usage-instructions)
     - [Terra => Ethereum / BSC / HMY](#terra--ethereum--bsc--hmy)
       - [原生资产](#native-assets)
       - [CW20 代币](#cw20-tokens)
     - [以太坊 / BSC / HMY => Terra](#ethereum--bsc--hmy--terra)
...
要使用 Shuttle 将资产从 Terra 转移到 Ethereum 或 BSC，请将资产发送到交易中的 Shuttle 地址，该交易的备注字段设置为目标链上的收件人地址。

使用下表为您的源和目标链对找到相应的 Shuttle 地址。
...
要传输本地 Terra 资产，例如 LUNA、UST、KRT 等，请创建一个 `MsgSend` 消息，其中收件人被设置为正确的 Shuttle 地址。

**包含`MsgSend`的交易示例:**

下面的`StdTx`从`columbus-5`上的`terra1rk6tvacasnnyssfnn00zl7wz43pjnpn7vayqv6`发送100个LUNA到`0x320BC76961fB4e2A0e2E86D43d4b9D13B4，通过主以太坊主网络Shuttlenet`9D13B4。
...
mAssets 和 MIR 令牌必须通过使用“MsgExecuteContract”调用令牌合约以不同方式发送。

**处理消息 JSON:**

必须将接收方设置为源/目标链对的适当 Shuttle 地址:
...
以下交易将 100 个 MIR 代币从 `columbus-5` 上的 `terra1rk6tvacasnnyssfnn00zl7wz43pjnpn7vayqv6` 发送到以太坊主网上的 `0x320BC76961fB4e2A0e2E86D43d4b9D13B4985b8f。
...
使用 bech32 解码的 terra 地址执行 `burn(uint256 amount, bytes32 to)`
